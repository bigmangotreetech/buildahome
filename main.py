...
def project_contractor_info():
    ...
    get_bills_query = 'SELECT w.stage, w.percentage, b.amount, b.approval_2_amount, b.trade, b.approved_on, b.cleared_balance, b.id' \
                        ' FROM wo_milestones w LEFT OUTER JOIN wo_bills b ON TRIM(b.stage)=TRIM(w.stage) AND b.contractor_code=%s AND b.project_id=%s AND trade=%s WHERE w.work_order_id=%s'
                ...
                for j in range(len(sub_task_list)):
                    ...
                    if len(task_name[0]) > 0:
                        task_item['sub_tasks'].append({
                            'name': task_name[0],
                            'is_complete': len(i[10].strip()) > 0 and  j < len(progress_list)
                        })
