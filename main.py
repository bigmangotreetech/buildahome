...
def view_project_details():
    if request.method == 'GET':
        ...
        get_details_query = 'SELECT '+fields_as_string+' from projects p LEFT OUTER JOIN App_users u on p.sales_executive=u.user_id AND p.project_id='+str(request.args['project_id'])
        cur = mysql.connection.cursor()
        cur.execute(get_details_query)
        result = cur.fetchone()
        details = {}
