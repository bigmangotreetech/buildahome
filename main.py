...
def project_contractor_info():
    ...
    get_bills_query = 'SELECT w.stage, w.percentage, b.amount, b.approval_2_amount, b.trade' \
                        ' FROM wo_milestones w WHERE w.work_order_id=%s LEFT OUTER JOIN wo_bills b ON b.contractor_code=%s AND b.project_id=%s'
    cur.execute(get_bills_query, (str(data['work_order_id']), contractor_code, project_id))
    bills = cur.fetchall()
